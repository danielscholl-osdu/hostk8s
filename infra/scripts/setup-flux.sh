...
apply_stamp_yaml() {
    ...
    if [ -f "$yaml_file" ]; then
        ...
        # Check if this file needs template processing (extension stacks or repository.yaml files)
        if [[ "$yaml_file" == *"extension/"* ]] || [[ "$yaml_file" == *"repository.yaml" ]]; then
            log_debug "Processing template variables for $yaml_file"
