...
class StackDeployer:
    ...
    def remove_stack(self, stack_name: str) -> None:
        ...
        logger.info(f"Checking for stack '{stack_name}' kustomizations...")
        ...
        if not stack_kustomizations:
            logger.info(f"No kustomizations found for stack '{stack_name}'")
            logger.info("Nothing to remove - stack is already clean")
            return

        logger.info(f"Found kustomizations for stack '{stack_name}' - proceeding with removal")
        ...
        result = run_kubectl(['get', 'kustomization', f'bootstrap-{stack_name}',
                           ...
                           check=False, capture_output=True)
        if result.returncode == 0:
            logger.info(f"Removing bootstrap kustomization: bootstrap-{stack_name}")
        ...
        logger.info(f"Removing all kustomizations for stack '{stack_name}'...")
        ...
        if stack_name.startswith('extension/'):
            logger.info("Cleaning up extension GitRepository...")
            run_kubectl(['delete', 'gitrepository', 'extension-stack-system',
                        '-n', 'flux-system'], check=False)
        else:
            logger.info(f"Cleaning up stack-specific GitRepository: flux-system-{stack_name}")
            run_kubectl(['delete', 'gitrepository', f'flux-system-{stack_name}',
                        '-n', 'flux-system'], check=False)

            # Check if any component kustomizations remain (from any stack)
            logger.info("Checking if flux-system GitRepository is still needed...")
            result = run_kubectl(['get', 'kustomizations', '-n', 'flux-system',
            ...
            if component_count == 0:
                logger.info("No component kustomizations remaining, removing shared GitRepository")
                run_kubectl(['delete', 'gitrepository', 'flux-system',
                            '-n', 'flux-system'], check=False)
            else:
                logger.info(f"Found {component_count} component kustomization(s) remaining, keeping shared GitRepository")

        logger.success(f"Software stack '{stack_name}' removal initiated")
        logger.info("Flux will complete the cleanup automatically (may take 1-2 minutes)")
        logger.info("Monitor with: kubectl get all --all-namespaces | grep -v flux-system")
    ...
    def wait_for_gitrepository_sync(self) -> None:
        ...
        logger.info("Waiting for GitRepository to sync...")
        timeout = 60
        while timeout > 0:
            ...
            if 'True' in result.stdout:
                logger.info("GitRepository synced successfully")
                break
            time.sleep(2)
            timeout -= 2
    ...
    def deploy_stack(self, stack_name: str) -> None:
        ...
        if not self.flux_is_installed():
            logger.info("Flux not found. Installing Flux first...")
            if not self.run_script('setup-flux'):
                logger.error("Failed to install Flux")
                sys.exit(1)
        else:
            logger.info("Flux is already installed and running")

        logger.info(f"Deploying software stack '{stack_name}'...")
        ...
        if uses_components:
            ...
            if source_component.exists():
                self.apply_stack_yaml(source_component,
                                    f"Configuring components repository for stack: {stack_name}",
                                    stack_name)
        ...
        if source_stack.exists():
            self.apply_stack_yaml(source_stack,
                                f"Configuring stack repository for stack: {stack_name}",
                                stack_name)
...
"""
            ...
            if bootstrap_yaml.exists():
                self.apply_stack_yaml(bootstrap_yaml,
                                    "Setting up GitOps bootstrap configuration",
                                    stack_name)
        ...
        logger.info(f"Software stack '{stack_name}' deployment completed!")
        logger.success(f"Software stack '{stack_name}' deployed successfully!")
...
Examples:
  ...
  %(prog)s extension/my-stack    # Deploy extension stack
    ...
    try:
        if operation == 'down':
            logger.info(f"Removing software stack '{stack_name}'...")
