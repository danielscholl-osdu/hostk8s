...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($applyExitCode -eq 0) {
            ...
            Write-Host "[$timestamp] [Registry] Error: $applyOutput"
            # Check nginx logs for specific errors
            $cmd = "kubectl logs -n ingress-nginx deployment/ingress-nginx-controller --tail=5 2>&1"
            $nginxLogs = Invoke-Expression $cmd | Select-String -Pattern "error|Error|ERROR"
            if ($nginxLogs) {
                Write-Host "[$timestamp] [Registry] Nginx controller errors: $nginxLogs"
            }
        }
    } else {
