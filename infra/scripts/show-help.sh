...
printf "  ${CYAN}sync${RESET}             Force Flux reconciliation (Usage: make sync [stack-name] or REPO=name/KUSTOMIZATION=name make sync)\n"
printf "  ${CYAN}suspend${RESET}          Suspend GitOps reconciliation (pause all GitRepository sources)\n"
printf "  ${CYAN}resume${RESET}           Resume GitOps reconciliation (restore all GitRepository sources)\n"
printf "\n"
...
printf "  ${CYAN}secrets-generate${RESET} Generate ephemeral secrets for a stack (Usage: make secrets-generate <stack-name>)\n"
printf "  ${CYAN}secrets-show${RESET}     Show secrets for a stack (Usage: make secrets-show <stack-name>)\n"
printf "  ${CYAN}secrets-clean${RESET}    Clean secrets for a stack (Usage: make secrets-clean <stack-name>)\n"
printf "\n"
